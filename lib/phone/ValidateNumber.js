...
function ValidateNumber() {
    ...
    self.lookupNumber = function (mobilenumber, callback) {
        ...
        console.log("lookupNumber..", mobilenumber);
        ...
        var xml_payload = `<xml><action>${payload.action}</action><login>${payload.login}</login>` +
            `<md5>${payload.md5}</md5><key>${payload.key}</key>` +
            `<destination_msisdn>${payload.destination_msisdn}</destination_msisdn></xml>`;
    ...
    self.lookupNumbers = function (contacts, callback) {
        var count = 0;
        var chunks = contacts.splitChunks(10000);
        ...
        chunks.forEach((chunk, i) => {
            ...
            (function processNext() {
                ...
                if (!c) {
                    ...
                    self.lookupNumber(c.numbers, response => {
                        for (var x  in response.TransferTo)
                            c[x] = response.TransferTo[x][0];
    ...
    self.check = function(csvFile, processNext){
        ...
        self.lookupNumbers.retryApply(this, contacts.isReady, 100, 1000, [
            contacts.contacts, function (c, i, last) {
                writeCsv(c, csvFile.replace(".csv", `-(${i}).csv`));
