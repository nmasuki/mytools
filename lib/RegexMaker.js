...
setInterval(function () {
    allDocs.filter(d => d.needsUpdate).forEach(d => d.save(function () {
        allDocs = allDocs.filter(_d => _d.id != d.id);
    }));
}, 10000);
    ...
    function etlCarrierLookUp(since, done) {
        if (!since) {
            function getSinceAndContinueLoadingSQL() {
                processed = 0;
                if (since)
                    return etlCarrierLookUp(_since, done);
            ...
            function processPending() {
                return CarrierLookUp.find({ processed: false, createdDate:{'$gt': new Date(since || '2014-01-01')} })
                    .limit(BATCHSIZE)
                    .sort({ createdDate: 1 })
                    .exec((err, docs) => {
                        if (err)
                        ...
                        if (docs.length) {
                            var dates = docs.filter(doc => doc.createdDate).map(doc => doc.createdDate);
                            var lastModified = dates.length ? dates.max(d => d.getTime()) : new Date('2014-01-01').getTime();
                            since = new Date(lastModified).toISOString();

                            var promise = new Promise((fulfill, reject) => {
                                var interval = setInterval(() => {
                                    if (processQueeue.length() < CONCURRENT*10) {
                                        processQueeue.push(docs.pop());
                            ...
                            return promise.then(() => processPending());
                        }
        ...
        if (processQueeue.length() > BATCHSIZE * 0.5) {
