...
function RegexMaker() {
    ...
    function etlCarrierLookUp(since, done) {
        if (!since) {
            return CarrierLookUp.find()
                ...
                .exec((err, docs) => {
                    ...
                    docs.forEach(carrier => {
                        if (!carrier.processed)
                    ...
                    console.log("Loaded " + processQueeue.length() + " unprocessed lookup from cache..");
        ...
        if (!con) {
            ...
            return connectMySQLConnection().then(c => {
                ...
                etlCarrierLookUp.apply(that, args);
            });
        }
        ...
        var query = `SELECT TOP (${CONCURRENT*5}) * FROM [Users]..[MobileNumberCarrierLookup] WHERE ModifiedDateTime > '${since}' AND MNC IS NOT NULL AND MCC IS NOT NULL ORDER BY ModifiedDateTime ASC;`
        con.query(query).then(result => {
            console.log("Loading...", result.recordset.length + " more.");
            result.recordset.forEach((data, i) => {
                if (data.JsonResult) {
            ...
            return connectMySQLConnection().finally(() => etlCarrierLookUp.apply(that, args));
        });
    }

    function updateCarrierRegex() {
        PhoneNumberRegex
            .find({
            ...
            .exec((err, regex) => {
                ...
                Promise.all(updates.map(c => c.save())).then(s => {
                    console.log("Done!");
                }).catch(err => {
    ...
    this.updateCarrier = updateCarrierRegex;
    ...
    this.updateRegex = function updateRegex(since, done) {

        if (updatesPromise && updatesPromise.isRunning) {
            var that = this,
                args = arguments;
            return updatesPromise.finally(() => updateRegex.apply(that, args));
        }

        var count = allDocs.length;
        //console.log(`Updating ${allDocs.length} records..`);

        if (!since && !done)
            done = updateCarrierRegex;
        else if (count <= 0) {
            if (typeof done == "function")
                done();
        ...
        updatesPromise = Promise.all(allDocs.map(d => {
            return d.save().then(a => {
                a.failed = false;
                return a;
            }).catch(err => {
                d.failed = (d.failed || 0) + 1;
                console.log("Error while saving..", err.message)
            });
        })).then(d => {
            var s = d.filter(i => i && !i.failed);
            if (s && count) {
                console.log(`Updated ${s.length}/${count} records..`);
                s.forEach(a => allDocs = allDocs.remove(a));
            } else {
            ...
            if (updatesPromise.isRunning) {
                updatesPromise.isRunning = false;
                updateRegexDb(since, () => updateRegex.apply(this, arguments));
            } else {
                console.log("Aiye...");
            }
        });

        updatesPromise.isRunning = true;
