        ...
        self.makeRequest = function (url, payload, method, headers) {
            ...
            if (Object.keys(payload).length) {
                ...
                if (method.toUpperCase() == "GET")
                    ...
                    curl += ` -X ${method.toUpperCase()} -d '${JSON.stringify(payload)}' ${url}`;
            }
                    ...
                    try {
                        ...
                        if (typeof body == "string" && body.trim()[0] == "<")
                            xml2js.parseString(body, (err, data) => {
                                options.fulfill(Object.assign(data, err));
                            });
                        else
                            options.fulfill(Object.assign(body));
                    } catch (e) {
                        options.reject(Object.assign({status: "error", error: e}, body))
                    }
                },
                error: function (error) {
                    console.log("HTTP call failed!", curl);

                    var body = JSON.parse(error.responseText) || error.responseText;
                    body = Object.assign({status: "error"}, body)
                    options.reject(body);
                }
            };
