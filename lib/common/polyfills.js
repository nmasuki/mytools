...
if (!console.olog) {
    function getLogMsg(args) {
        ...
        return msg.replace(',"$id":1', '').replace('"$id":1', '').replace(/^\s*(\{\s*})\s*$/, '');
    }
    ...
    console.debug = function () {
        ...
        if (msg.trim()) {
            try {
                ...
                //Ignore
                msg = msg.trim().split('\t').first() + '\r\n' + e.message;
            }
            console.log(msg.trim().split('\t').first());
        }
    }
        ...
        function censor(val) {
            ...
            return function (key, value) {
                if (typeof value === 'object' && value !== null) {
                    ...
                    if (!value["$id"]) {
                        //Use $id to for track Circular reference
                        value["$id"] = cache.length + 1;
                ...
                if (++i > 99) // seems to be a harded maximum of 100 serialized objects?
                    return '[Unknown]';
