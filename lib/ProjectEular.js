...
module.exports.Eular.problem5 = function LCM(options) {
    ...
    var largerSets = groupedPrimes
        .distinctBy(function (a) { return a[0]; });
...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48`
    ...
    var selected = [[6, 8], [7, 9], [8, 10], [9, 11]];
    var seq = selected.map(s => set[s[0]][s[1]]);
    var prod = seq.prod();

    var fn = p => Math.sequence(0, 3, i => [p[0] + i, p[1] + i]);
    var adj = p => Math.sequence(-1, 1,
        i => Math.sequence(-1, 1,
            j => {
                var pt = [p[0] + i, p[1] + j];
                if (selected.any(x => x[0] != pt[0] || x[1] != pt[1]))
                    return pt;
                else
                    return [];
            }).filter(a => a.length)).flatten();

    return (adj([6, 8]));
    var trans = selected.map(adj);
    trans = trans.map(fn);
    return trans;

    var prods = trans.map(t => t.prod()).sort((a, b) =>  a - b);
