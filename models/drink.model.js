...
var regexPostDate = /(\w{3}\s\d{1,2},{0,1}\s*\d{4}\s*.{0,3}\.{3}.{0,4})/
    ...
    if (!useSyncGateway) {
        bucket.get(_id, function (err, row, meta) {
            if (!err) {
                ...
                setTimeout(get, 1000 + Math.random() * 10000, _id, callback);
            } else if (err.code == 13) {
                callback()
            } else {
...
function save(_id, _rev, data) {
    get(_id, function (err, row, cas) {
        ...
        if (!useSyncGateway) {
            ...
            najax.put({
                ...
                headers: {
                    ...
                    "Origin": "http://${host}:4985",
                    "Proxy-Connection": "keep-alive",
                    "Referer": `http://${host}:4985/_admin/db/${database}/documents/product-cellar-cask-2-litres`,
                    "User-Agent": "Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.181 Safari/537.36"
                },
                success: function (data) {
                ...
                error: function (xhr, status, err) {
                    ...
                    if (err.code == "ETIMEDOUT" || err.code == "ECONNRESET") {
                        ...
                        setTimeout(save, 5000 + Math.random() * 10000, _id, _rev, doc);
                    }
                    else if (typeof callback == "function")
                        callback()
                }
...
function fillDescription(_id, _data) {
    ...
    if (!hasDescription) {
        googleSearch(name.trimRight('s') + ' is a', function (data, err) {
            if (err) {
                console.log(err);
                return;
            }
            ...
            save(_id, _data._rev, _data);
        });
    }
}

function webhoseSearch(content, callback) {
...
function msnSearch(content, callback) {
    var searchId = content.toLowerCase().replace(/[\W]+/g, '-').trim('-');
...
Drink.syncSave = save;
Drink.syncGet = get;
Drink.syncFillDescription = fillDescription;
Drink.syncFillIn = fillIn
